...
class TestDocuSignIntegration:
    ...
    async def test_generate_contract_pdf_html_fallback(self, mock_loader, mock_supabase, mock_contract):
        ...
        with patch('backend.services.sign_service.Environment') as mock_env:
            ...
            mock_storage.from_.return_value.get_public_url.return_value.data = {'publicUrl': 'https://test.com/contract.html'}
    ...
    async def test_docusign_create_envelope(self, mock_envelopes_api, mock_contract, mock_client_data, mock_lawyer_data):
    ...
    async def test_docusign_get_envelope_status(self, mock_envelopes_api):
        ...
        mock_envelope.envelope_id = "test-envelope-123"; mock_envelope.status = "completed"
        mock_envelope.created_date_time = "2025-01-01T10:00:00Z"; mock_envelope.completed_date_time = "2025-01-01T15:00:00Z"
        
        mock_recipients = MagicMock()
        signer1 = MagicMock(); signer1.name = "João Silva"; signer1.email = "cliente@test.com"; signer1.status = "completed"; signer1.signed_date_time = "2025-01-01T12:00:00Z"
        signer2 = MagicMock(); signer2.name = "Dr. Maria Santos"; signer2.email = "advogado@test.com"; signer2.status = "completed"; signer2.signed_date_time = "2025-01-01T15:00:00Z"
    ...
    async def test_fallback_on_docusign_error(self, mock_create_client, mock_contract):
        ...
        with patch.object(settings, 'USE_DOCUSIGN', True), \
            ...
            mock_ds_instance.create_envelope.side_effect = Exception("DocuSign API Error")
            mock_ds_class.return_value = mock_ds_instance
            
            service = SignService()
            service._generate_simple_html_contract = AsyncMock(return_value='https://fallback.com/contract.html')
            
            result = await service.generate_contract_pdf(mock_contract)
            
            assert result == 'https://fallback.com/contract.html'
            service._generate_simple_html_contract.assert_called_once_with(mock_contract)
    ...
    async def test_jwt_authentication_flow(self, mock_api_client):
        ...
        with patch.object(settings, 'DOCUSIGN_API_KEY', 'test-integration-key'), \
            ...
            with patch('backend.services.sign_service.load_pem_private_key'), \
                ...
                mock_api_client.return_value.set_default_header.assert_called_with("Authorization", "Bearer test-access-token")
    ...
    async def test_error_handling_and_logging(self, mock_create_client, mock_contract):
        ...
        with patch.object(settings, 'USE_DOCUSIGN', True), \
            ...
            service = SignService()
            service._generate_simple_html_contract = AsyncMock(return_value='fallback.html')
            ...
            mock_print.assert_any_call("Erro no DocuSign, usando fallback HTML: Erro ao criar envelope DocuSign: Network error")
