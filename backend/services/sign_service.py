...
class SignService:
    ...
    async def _upload_html_to_storage(self, file_path: str, filename: str) -> str:
        ...
        try:
            ...
            # Obter URL pública
            public_url = self.supabase.storage.from_('contracts').get_public_url(bucket_path)
    ...
    async def _get_lawyer_data(self, lawyer_id: str) -> Dict[str, Any]:
        ...
        result = self.supabase.table('profiles').select('*').eq('id', lawyer_id).single().execute()
        return result.data if result.data else {}
    
    async def _get_client_data(self, client_id: str) -> Dict[str, Any]:
        """Busca dados do cliente"""
        result = self.supabase.table('profiles').select('*').eq('id', client_id).single().execute()
        return result.data if result.data else {}
    ...
    async def send_signature_notification(self, contract: Contract, role: str):
        ...
        try:
            ...
            print(f"Notificação: {user_data.get('full_name')} precisa assinar o contrato {contract.id}")
    ...
    def get_signer_info(self, recipients: List[Dict[str, Any]], user_email: str) -> Optional[Dict[str, Any]]:
